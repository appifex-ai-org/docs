...
- **Nodes:** a searchable list of the same nodes, grouped by type.
...
- **Rollout health:** counts for orphan nodes, stale sources, low-confidence edges, review backlog, and extraction jobs.
- **Review queue:** proposed graph items waiting for confirmation, rejection, or escalation.

**Run extraction** queues graph extraction for the current article. It is not an agent chat turn. Vokal reads the article and proposes nodes and relationships for the Knowledge Base graph. Proposed items can appear in the review queue before they become confirmed graph context.
